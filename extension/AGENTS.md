...
### タイマーURL（フロントのタイマー画面向け）

`video_operation` に任意 `title` を同梱する（通常操作・5s ハートビート双方）。フロントの
タイマー画面（拡張機能なしのユーザー）が既存ブロードキャストからエピソード切替に追従して
タイトルを表示するため。旧バックエンドは無視するため後方互換。共有パネル（`SharePanel`）には
